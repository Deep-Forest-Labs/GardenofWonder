...
| **Whole plants + growth stages** | `flora.js` `Flora.plant(seed)`; stages in `style.css` | one `data-stage` attribute, 0–3, from `stageOf()` in `ui.js` | 19 plants × **3 visible stages** = 57 appearances, but only **19 SVG strings** | [`plant-rose.svg`](../art/exports/samples/plant-rose.svg) |
