...
| 0–340 ms | **The gold wallet** | `walletPop`, 340 ms: swells to **1.198 and tips −3.3° at 78 ms**, dips to 0.982 at 253 ms, rests at 340. It pops at the tap — **before** the coins arrive. | `popWallet()` — grep `c.wallet.classList.add('pop')` |
...
| 900 ms | **Hold** | A finger that stays down repeats the whole tap — everything above, at the new combo pitch — every 900 ms (Quick Grip: 60 ms less per level, never under 180 ms). The interval is read once, at the press. | `wireFlower()` — grep `holdTimer = setInterval` |
