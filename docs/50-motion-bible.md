...
1. **The Year panel's ready ring has no substitute** (the dock button has one; the panel's words
   still say it).
2. **Four quiet fades are 80 ms steps**, because their durations lack `!important`: the aurora's
   exit, a front's arrival and parting, the sky's wash (a tint jump of up to 0.68 opacity), the
   sunbreak's ending.
3. **Cancels that lose on specificity**, which the clamp alone rescues: the wet glint and the storm
...
4. **A cancel written above its rule**: the Hollow's loadout dim still fades, over 80 ms.
5. **A delay survives the clamp**: the aurora's first ribbon still waits 1.2 s.
6. **The procs vanish** — the Rain Dance cloud and drops, the bee, a creature's motes. Their float,
   sparks and sound still say what happened, so no state is lost.
7. **Some still poses are brighter than any moving frame**: the Hollow's dust, shaft and glow and
   the flower's glow rest at full opacity. A still pose should be chosen, not inherited.
8. **The reduced Tally stacks its sounds**: every line's `coin` and buzz within a few ms.
9. **The sheet's scrim lingers** 340 ms after an 80 ms close (**read**).

Fixed this round (docs/11): the adjacency flash's missing `!important` duration, the ripe plot's
stuck white band, and the meadow's vanishing "affordable here" cell.
...
_Read out of `style.css` and the JavaScript on this run: **103 keyframes**, played by **121 uses in 116 rules**; **106 rules that set a transition** (120 transitions, plus longhand overrides); **203 calls into `FX`** across 8 files; and **32 pieces of JavaScript choreography**. Every keyframe is played by something and every rule plays a keyframe that exists — the run refuses to write this block otherwise. No reduced-motion answer carries a ⚠ mark._
...
| `leafWaveR` | `transform` at 0 50 100% | `2.9s` | `ease-in-out` | ∞ | `.tf-leaf-r` | on by default<br>swapped for `wxLeafHold` where `#game:is([data-weather="rain"],[data-weather="storm"]):is([data-wx-phase="transform"],[data-wx-phase="linger"]) .tf-leaf-r` | clamp: one .001ms run, then its own style |
...
| `sweep` | `transform` at 0 100% | `1.9s` | `linear` | ∞ | `.plot[data-state="ready"] .plot-inner::before` | ancestor `.plot[data-state="ready"]` | cancelled by `.plot[data-state="ready"] .plot-inner::before`, which also declares `opacity:0` |
...
| `verbLinkCalm` | `opacity` at 0 10 70 100% — declared inside `(prefers-reduced-motion:reduce)` | `1.6s` | `linear` | ×1 · fill forwards | `.plot.verb-source::after`<br>`.plot.verb-linked::after` | reduced motion — self `.verb-source`<br>reduced motion — self `.verb-linked` | reduced-motion substitute — its `1.6s` is `!important` and outlives the clamp |
...
| `wxLeafHold` | `transform` at 0 50 100% | `2.8s` | `ease-in-out` | ∞ | `#game:is([data-weather="rain"],[data-weather="storm"]):is([data-wx-phase="transform"],[data-wx-phase="linger"]) .tf-leaf-r` | ancestor `#game:is([data-weather="rain"],[data-weather="storm"]):is([data-wx-phase="transform"],[data-wx-phase="linger"])` | cancelled by `#game:is([data-weather="rain"],[data-weather="storm"]):is([data-wx-phase="transform"],[data-wx-phase="linger"]) .tf-leaf-r` |
...
| `#game:is([data-weather="rain"],[data-weather="storm"]):is([data-wx-phase="transform"],[data-wx-phase="linger"]) .tf-leaf-r` | `transform` | `.55s` | `cubic-bezier(.34,1.56,.64,1)` | — | ancestor `#game:is([data-weather="rain"],[data-weather="storm"]):is([data-wx-phase="transform"],[data-wx-phase="linger"])` | the same properties over .08s (the clamp) |
...
| `ease-in-out` | 70 | 0 |
| `linear` | 19 | 10 |
| `cubic-bezier(.34,1.56,.64,1)` | 12 | 6 |
| `ease-out` | 7 | 3 |
...
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-feed]` | `FX.sparks(at.x, at.y, 12, got.def.art.glow)` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-feed]` | `FX.stars(at.x, at.y, 5, '#ffe066')` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-feed]` | `FX.haptic(10)` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-deliver]` | `FX.stars(at.x, at.y, 8, '#ffe066')` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-deliver]` | `FX.sparks(at.x, at.y, 16, who ? who.art.accent : '#ffd6e8')` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-deliver]` | `FX.coins(at.x, at.y, 10)` |
...
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-craft]` | `FX.sparks(at.x, at.y, 10, '#8ce0ff')` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-craft]` | `FX.shake(4)` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-sell]` | `FX.coins(at.x, at.y, 8)` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-sell]` | `` FX.float(at.x, at.y - 6, `+${fmt(total)}`, 'big') `` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-ad]` | `FX.sparks(at.x, at.y, 12, '#b197fc')` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-ad]` | `FX.ring(at.x, at.y, '#ffffff', 0.45, 70)` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-ad]` | `FX.shake(4)` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-buy]` | `FX.sparks(at.x, at.y, 12, '#ffe066')` |
| `ui-sheet.js` | `(module level)` › `el.sheetBody on 'click'` › `[data-buy]` | `FX.ring(at.x, at.y, '#ffffff', 0.45, 70)` |
