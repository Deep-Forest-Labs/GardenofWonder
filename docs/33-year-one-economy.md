...
2. **The mint reads earnings, never balance.** `state.year.coinsEarned` is a
   lifetime-this-year accumulator, incremented wherever credits are granted, never decremented.
   Spending is seed-neutral by construction, and a sim-test asserts it (buy everything one tick
   before the Turn; the pouch must not change).
3. **Cheated grants are excluded from the mint.** The dev/cheat grant paths skip
   `coinsEarned`. Testers keep their buttons; the pacing data stays clean.
...
```
pouch = round( DATA.year.mintK × sqrt(coinsEarnedThisYear) × (1 + DATA.year.veterancy × turnsCompleted) )
mintK = 0.1        veterancy = 0.2        YEAR.minSeeds = 10   // the meter's turn threshold
```

First Turn pays **~61 seeds** on a typical ~160K-coin first year. The blessing grants **one free
petal** on a chosen flower, outside the pouch. The two growth exponents that must stay matched:
...
Year one's quest ladder (777 rep, level 17) is untouched and remains the tutorial. The four
meadow-dependent quests (`q_hive_1`, `q_honey_3/8/15`, 114 rep) stay live **only if** the meadow
remains reachable in the Year build from day one — it does (Summer's edge, unchanged), so no
pause is needed. Re-check this the moment the meadow's entry point moves.
...
**The windfall rule** (`DATA.fall.windfall = 0.5`): harvesting a Fall bed whose every planted
plot is ripe pays **+50% on the whole bed**. One knob, one legible rule, and it is what makes
Fall an appointment rather than a chore. Fall plants gate on gold prices only in v1 — no unlock
walls inside Fall until its list grows.
...
the game's loudest celebration short of a Wonder. Ships with Fall if slice A wants its
screenshot; numbers deliberately absurd and data-tagged.
...
- **Existing saves enter the Year mid-flight:** current garden becomes year one in progress;
  `coinsEarned` backfills from `stats` totals where honest, else starts at zero with the meter
  low — never negative surprise. Nobody loses a seed they could plant: every seed the save has
  planted or afforded is marked unlocked free, the grandfather pattern from the level-gate
  migration.

## The sim-test bill

The suite grows with slice A; these are the invariants, most inherited from the audit:

1. The never-resets partition, asserted field by field across a Turn.
2. The Turn kills no running growth timer, in any season.
3. Mint reads `coinsEarned` only; a spend-everything-then-Turn run pouches identically.
4. Cheat grants never reach `coinsEarned`.
5. `yield = cost × 1.4` for every seed including Fall's; petals leave the curve alone.
6. Gems/hour flat across all seeds; no petal changes any gem chance.
7. Mutation income share stays in band with Storm-Kissed at cap.
8. Quick Sprout at cap + Sprinklers + Keeper + Seed Rush stays above the 0.3 floor.
9. Stand slots regenerate at the Turn; no post-Turn order names an unaffordable bloom.
...
12. The windfall pays only on a fully-ripe bed, and only once per bed fill.
