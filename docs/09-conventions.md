...
10. **Enable reduced motion** and confirm every state that animates still *reads* when it does not — not merely that things calm down. `node tools/probe.js media:reduce page:index.html …` screenshots it. **Every** variant, enumerated, not one sample: a variant that names its own keyframe is more specific than the rule you just fixed and keeps animating, so `.float.crit` stayed invisible while every one of its siblings was repaired. Float them all in one `eval:` step and read each one's computed `opacity` and `animation-name` as a table — then check with the preference off, so the fix has not deleted a real animation.
