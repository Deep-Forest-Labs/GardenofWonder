...
> screens and the asset inventory. The first half is authored: the four principles with their numbers,
> **the tap frame by frame — driven, not read**, the particle system as a spec (`coins(x, y, 8)` can be
> rebuilt from its paragraph, and an engineer-critic did), doc 06's feedback ladder carried over
> verbatim and extended to every moment of feel, the sequences, reduced motion and its static
> substitutes. The second half is generated by **`node tools/export-motion.js`** — every keyframe,
> transition, `FX` call and piece of JavaScript choreography, with what switches each on or off and
> what reduced motion does to it, worked out from the cascade. **`--check` goes red when it is stale**,
...
> the tap's pitch wraps every ten, so a player at the combo cap hears the lowest note on every tap; and
> doc 06's ladder disagrees with the code in ten cells — contract or code, per cell. Doc 05's stale
> animation count now points at the generated one. Reasoning and what was rejected: the 2026-09-21
> (docs) entry in [10-decision-log.md](10-decision-log.md).
>
> **The two-minute check, on the phone:** open doc 50 on the wiki, read "The tap, frame by frame", and
> tap the flower with the page beside it — the head squashes and stretches, the mouth opens for a
> third of a second, the number pops up and is half faded within a third of a second, four coins hop
> and are pulled into the gold wallet.
...
| Item | What it is now | Commit |
| --- | --- | --- |
| **#26** (engine) | The gem-skip button refuses outright, spending nothing, when a plant's booked moment sits under a sky carrying a catch — any of the four, not just Wonderfall | `0655f8e` |
...
then invalid, and the whole rule is dropped — no error, and the text still reads as a rule. That is how
the flower's rain pose went missing for three weeks: deleting a `@keyframes` left one of its stops and
the closing `}` behind, and the rule after them never reached the game. **Read the stylesheet the
...
node tools/export-motion.js --check # is doc 50's motion inventory still true to style.css and the JS?
